...
export const ClaudeSettings = makeProviderSettingsSchema(
  ...
  {
    order: ["binaryPath", "apiBaseUrl", "apiKey", "homePath", "autoCompactWindow", "launchArgs"],
  },
);
...
export const KiroSettings = makeProviderSettingsSchema(
  {
    // Off by default like Grok: the ACP binding is new. Users opt in from Settings.
    enabled: Schema.Boolean.pipe(
      Schema.withDecodingDefault(Effect.succeed(false)),
      Schema.annotateKey({ providerSettingsForm: { hidden: true } }),
    ),
...
export const MuseSettings = makeProviderSettingsSchema(
  {
    ...
    binaryPath: makeBinaryPathSetting("muse").pipe(
      Schema.annotateKey({
        title: "Binary path",
        description: "Path to the Muse Code binary. Requires a build with muse serve support.",
