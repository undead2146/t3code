  ...
  const scanSummary = Effect.fn("UsageService.scanSummary")(function* (
    ...
    for (const {
      ...
      sources.push({
        ...
        // A directory with no live files reports missing even when the scan
        // produced a status; saved records still keep it an available source.
        // Clients exclude missing sources, so retained usage stays visible.
